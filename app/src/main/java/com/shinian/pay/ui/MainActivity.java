...
public class MainActivity extends AppCompatActivity implements OnLongClickListener {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        // 启动时自动检测全部必需权限（通知使用权/电池白名单/通知显示/文件读写/自启动），
        // 缺失时弹出清单对话框，点击「去开启」直达对应系统设置页，无需用户自己寻找
        PermissionGuideHelper.checkAndShowOnLaunch(this);

        // 读入保存的配置数据并显示（修复：字符串必须用 equals 内容比较，== 比较引用）
    ...
    protected void onResume() {
        super.onResume();
        // 用户从系统设置页授权返回后自动复查：全部授权则提示就绪，仍有缺失则继续引导
    ...
    private void exitApp() {
        try {
            ...
            // 3. 通过停止监听服务触发其 onDestroy（中断心跳线程、释放唤醒锁）
            try {
                Intent nlIntent = new Intent(MainActivity.this, PayNotificationListenerService.class);
                stopService(nlIntent);
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    ...
                    String urlPath = "https://w.t3yanzheng.com/A729B02347E855EC";
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
                ...
                public void onClick(DialogInterface dialog, int which) {
                    ...
                    if (fs.equals("微信打赏")) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            ...
                            //（旧逻辑在此检查 WRITE_EXTERNAL_STORAGE，该权限在 Android 13+ 已废弃，
                            //  会导致保存收款码功能静默失效）
        ...
        if (itemId == R.id.perm_check) {
            List<PermissionGuideHelper.PermissionItem> missing = PermissionGuideHelper.checkMissingPermissions(this);
    ...
    public void startQrCode(View v) {
        ...
        // 文件读写权限：仅 Android 12L 及以下需要申请（用于相册选图与保存图片）。
        // Android 13+ 已废弃 READ/WRITE_EXTERNAL_STORAGE：
        //  - 相册选图走系统选择器（自带临时授权，无需权限）
        //  - 保存图片走 MediaStore（无需权限）
        // 因此 13+ 直接进扫码页，不再做注定失败的申请
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == AppConstants.REQ_QR_CODE && resultCode == RESULT_OK) {
            ...
            // 使用统一网络客户端验证配置（带重试）
            NetworkClient.getWithRetry(tmp[0], "/appHeart?t=" + t + "&sign=" + sign, new Callback() {
                @Override
                public void onFailure(Call call, IOException e) {
                    Log.d(TAG, "配置验证请求失败: " + (e != null ? e.getMessage() : ""));
                }
    ...
    private void showPermissionDeniedDialog(String permName, String reason) {
        new AlertDialog.Builder(MainActivity.this)
                ...
                .setPositiveButton("去开启", (dialog, which) ->
                        PermissionGuideHelper.openAppDetailsSettings(MainActivity.this))
                .setNegativeButton("取消", null)
                .show();
    }
